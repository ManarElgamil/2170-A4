<!-- Code for login form and login form processing taken from group lab4 -->
...
<main class="container text-center"  >
  ...
  <form method="post"  action="login.php">
    ...
    <div class="form-floating">
      <input type="email" name="i-email" class="form-control mb-4" id="floatingInput" placeholder="name@example.com">
      <label for="floatingInput">Email address</label>
    </div>
    <div class="form-floating">
      <input type="password" name="i-password" class="form-control mb-4" id="floatingPassword" placeholder="Password">
      <label for="floatingPassword">Password</label>
    </div>

    <input type="hidden" name="token" class="form-control mb-4" value="<?php echo $_SESSION['token']; ?>">

    <a href="../register.php" class="nav-link">New to this MailYoda? Register here.</a>

    <button class="w-100 btn btn-lg btn-primary mb-4" name="submit-login-info" value="Log in" type="submit">Log in</button>
...
if (isset($_REQUEST['submit-login-info'])) {
  ...
  if ($_REQUEST['token'] == $_SESSION['token']){
      ...
      //sanitizing the data submitted from the form
      $email = sanitizeData($_REQUEST['i-email']);

      $password = sanitizeData($_REQUEST['i-password']);

      $sql = "SELECT * FROM `je_login` WHERE je_login_email='$email'";
      $result = $db->query($sql);
      ...
      if ($result->num_rows > 0){
        ...
        //getting user name
        $sql2 = "SELECT * FROM `je_users` WHERE je_user_login_id='$loginId'";
        ...
        $x = password_verify($password, $row['je_login_email']);
